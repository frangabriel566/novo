...
import { Plus, Search, Edit2, Trash2, Boxes, X } from 'lucide-react'
...
export default function WholesaleProductsPage() {
  ...
  const [editProduct, setEditProduct] = useState<WholesaleProduct | undefined>()
  ...
  const loadProducts = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/wholesale-products?${params}`)
  ...
  function openEdit(product: WholesaleProduct) { setEditProduct(product); setModalOpen(true) }

  async function handleDelete() {
    if (!deleteId) return
    setDeleting(true)
    try {
      await fetch(`/api/wholesale-products/${deleteId}`, { method: 'DELETE' })
      setDeleteId(null)
      loadProducts()
  ...
  return (
    <div>
      <Header
        ...
        subtitle={`${total} produto${total !== 1 ? 's' : ''} de atacado cadastrado${total !== 1 ? 's' : ''}`}
        actions={<Button onClick={openCreate}><Plus className="w-4 h-4" />Novo Produto</Button>}
      />
      ...
      <div className="px-4 lg:px-8 py-6 space-y-4 animate-fade-in">
        ...
        {lowStockFilter && (
          <div className="flex items-center gap-2 px-3 py-1.5 bg-amber-500/10 border border-amber-500/20 rounded-lg w-fit text-xs text-amber-300">
            ...
            <a href="/produtos-atacado" className="p-0.5 rounded hover:bg-amber-500/20 transition-colors"><X className="w-3.5 h-3.5" /></a>
          </div>
        )}
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
          {loading ? <PageLoader /> : products.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-gray-500 gap-3">
              ...
              <p className="text-base font-medium">Nenhum produto de atacado encontrado</p>
              <Button onClick={openCreate} size="sm"><Plus className="w-4 h-4" />Criar produto</Button>
            </div>
          ) : (
            <>
              {/* Mobile cards */}
                    ...
                    <div key={product.id} className="px-4 py-3 flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className={`inline-flex px-1.5 py-0.5 rounded-full text-xs font-medium border ${stock.color}`}>
                            {product.quantity} un
                          </span>
                        </div>
                      </div>
              ...
              <table className="hidden sm:table w-full">
                <thead>
                  <tr className="border-b border-gray-800">
                    ...
                    <th className="text-right px-6 py-4 text-xs font-semibold text-gray-400 uppercase tracking-wider">Preço</th>
                    <th className="text-center px-6 py-4 text-xs font-semibold text-gray-400 uppercase tracking-wider">Estoque</th>
                  ...
                  {products.map((product) => {
                    ...
                    return (
                      <tr key={product.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="px-6 py-4"><span className="text-sm text-gray-300">{product.category}</span></td>
                        <td className="px-6 py-4 text-right"><span className="text-sm font-semibold text-white">{formatCurrency(product.price)}</span></td>
                        <td className="px-6 py-4 text-center">
                          <div className="flex flex-col items-center gap-1">
                            <span className="text-sm font-medium text-white">{product.quantity}</span>
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${stock.color}`}>{stock.label}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button onClick={() => setDeleteId(product.id)} className="p-2 rounded-lg text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition-colors"><Trash2 className="w-4 h-4" /></button>
                          </div>
                        </td>
                      </tr>
                    )
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editProduct ? 'Editar Produto' : 'Novo Produto de Atacado'}>
        <WholesaleProductForm product={editProduct} onSuccess={() => { setModalOpen(false); loadProducts() }} onCancel={() => setModalOpen(false)} />
      </Modal>

      <Modal isOpen={!!deleteId} onClose={() => setDeleteId(null)} title="Remover Produto" size="sm">
        <p className="text-gray-300 text-sm mb-6">Tem certeza que deseja remover este produto? Esta ação não pode ser desfeita.</p>
