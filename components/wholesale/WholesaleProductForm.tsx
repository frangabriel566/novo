...
export default function WholesaleProductForm({ product, onSuccess, onCancel }: WholesaleProductFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const url = product ? `/api/wholesale-products/${product.id}` : '/api/wholesale-products'
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <Input
        ...
        placeholder="Caixa de Refrigerante (12un)"
        required
      />
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <CurrencyInput label="Preço de atacado (R$)" value={priceCents} onChange={setPriceCents} required />
      </div>
