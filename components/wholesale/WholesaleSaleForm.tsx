...
import { Plus, Trash2, Boxes, Search, Banknote, ArrowLeftRight } from 'lucide-react'
import Button from '@/components/ui/Button'
import Input from '@/components/ui/Input'
import Select from '@/components/ui/Select'
import Textarea from '@/components/ui/Textarea'
import { Customer, WholesaleProduct, WholesaleSaleItemInput } from '@/types'
...
export default function WholesaleSaleForm() {
  ...
  const [items, setItems] = useState<WholesaleSaleItemInput[]>([])
  ...
  useEffect(() => {
    Promise.all([fetch('/api/customers?pageSize=200'), fetch('/api/wholesale-products?pageSize=200')])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/wholesale-sales', {
      ...
      router.push('/vendas-atacado'); router.refresh()
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 sm:p-6 space-y-4">
        ...
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 text-gray-500 gap-2">
            <Boxes className="w-10 h-10 text-gray-700" />
